...
pub fn part2(xs: Vec<&str>) -> &str {
    ...
    let xmax = mapped.iter().map(|&(x, y)| x).max().unwrap();
    let ymax = mapped.iter().map(|&(x, y)| y).max().unwrap();
    for y in 0..=ymax {
        for x in 0..=xmax {
