defmodule Day04 do
  ...
  def calculate_sleep_time([head|tail], current, result) do
    ...
    cond do
      ...
      String.contains?(head, "wakes up") ->
        calculate_sleep_time(tail,
                             ...
                             Map.merge(result, %{elem(current, 0) => Map.new(elem(current, 1)..get_minute(head)-1, fn x -> {x, 1} end)}, fn _k, v1, v2 ->
    ...
    def solve(records) do
      ...
      {topid, _} = data |> Map.to_list |> Enum.map(fn {k, v} -> {k, sleep_minutes(v)} end)
    ...
    def solve2(records) do
      data = records |> sort_chronological |> calculate_sleep_time({}, Map.new)
      r = data |> Map.to_list |> Enum.map(fn {id, map} ->
        {id, (Map.to_list(map) |> Enum.max_by(fn {_, count} ->
        count
        end))}
      end) |> Enum.max_by(fn {_, {_, count}} -> count end)
      {id, {minute, _}} = r
      id * minute
    end
  end
