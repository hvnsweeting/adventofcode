defmodule Aoc2019Day2Test do
  ...
  test "Program 1,0,0,0,99 becomes 2,0,0,0,99 (1 + 1 = 2)." do
    assert Intcode.run("1,0,0,0,99") == Intcode.state_to_int_list("2,0,0,0,99")
  end

  test "Program 2,3,0,3,99 becomes 2,3,0,6,99 (3 * 2 = 6). " do
    assert Intcode.run(" 2,3,0,3,99 ") == Intcode.state_to_int_list(" 2,3,0,6,99 ")
  end

  test "2,4,4,5,99,0 becomes 2,4,4,5,99,9801 (99 * 99 = 9801)." do
    assert Intcode.run(" 2,4,4,5,99,0 ") == Intcode.state_to_int_list(" 2,4,4,5,99,9801 ")
  end

  test "1,1,1,4,99,5,6,0,99 becomes 30,1,1,4,2,5,6,0,99. " do
    assert Intcode.run("1,1,1,4,99,5,6,0,99 ") ==
             Intcode.state_to_int_list("30,1,1,4,2,5,6,0,99")
  end

  test "Program 1,9,10,3,2,3,11,0,99,30,40,50" do
    assert Intcode.run("1,9,10,3,2,3,11,0,99,30,40,50") ==
